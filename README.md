...
StratArena is an OpenEnv-compatible multi-agent reinforcement learning environment for theory-of-mind training under uncertainty. A learner competes against aggressive and conservative opponents for limited resources, sees only behavioral signals, and must infer hidden opponent state such as budget depletion and strategic posture.
...
uv run python training/training_script.py
```
