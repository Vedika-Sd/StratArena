...
class OpenAIPolicy:
    def __init__(self, model: str, api_key: str, base_url: str = API_BASE_URL):
        self.model = model
        self.client = OpenAI(api_key=api_key, base_url=base_url)

    def act(self, task: str, obs: StratArenaObservation, trace: list[StepTrace]) -> tuple[float, str]:
...
def run_episode(
    ...
    *,
    ...
    policy = OpenAIPolicy(model=model, api_key=api_key or API_KEY, base_url=base_url) if use_llm and (api_key or API_KEY) else None
...
def main() -> None:
    ...
    if len(scores) > 1:
        ...
        print("[DEBUG] SUMMARY " + " | ".join(f"{task}={score:.3f}" for task, score in scores.items()) + f" | avg={average:.3f}")
